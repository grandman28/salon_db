...
$id_produs = isset($_GET['id']) ? intval($_GET['id']) : null;

if (!$id_produs) {
    die("ID produs invalid.");
}

$sql = "SELECT * FROM Produse WHERE ID_Produs = ?";
$params = [$id_produs];
...
$produs = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($numeprodus) && !empty($pretunitar)) {
        $sql = "UPDATE Produse 
                SET Nume = ?, Pret_Unitar = ?, [Cantitate_Flacon(ml)] = ?, [Pret/Kg] = ?
                WHERE ID_Produs = ?";
        $params = [$numeprodus, $pretunitar, $cantitate, $pretkg, $id_produs];
        ...
        if ($stmt === false) {
            ...
            $_SESSION['message'] = "Produsul a fost actualizat cu succes!";
                    ...
                    <div class="container" style="margin-top: 20px;">
                        ...
                        <form method="POST">
                            ...
                            <div class="row mb-2 mt-4">
                                ...
                                <div class="col-sm-6">
                                    <input type="text" class="form-control" id="nume_produs" name="nume_produs"  value="<?php echo $produs['Nume']; ?>"
                                        required>
                                </div>
                            </div>
                            ...
                            <div class="row mb-2">
                                ...
                                <div class="col-sm-2">
                                    <input type="number" class="form-control" id="pret_unitar" name="pret_unitar" value=<?php echo $produs['Pret_Unitar']; ?>
                                        step="0.01" min="0" required>
                                </div>
                            </div>
                            <div class="row mb-2">
                                <label for="cantitate" class="col-sm-2 col-form-label align-items-centerS">Cantitate flacon(ml):</label>
                                <div class="col-sm-2">
                                    <input type="number" class="form-control" id="cantitate" name="cantitate" value=<?php echo $produs["Cantitate_Flacon(ml)"]; ?>
                            ...
                            <div class="row mb-4">
                                ...
                                <div class="col-sm-2">
                                    <input type="number" class="form-control" id="pret_kg" name="pret_kg" step="0.01" value="<?php echo $produs['Pret/Kg']; ?>"
                                        min="0">
                                </div>
                            </div>
                            <div class="row mb-2">
                                ...
                                <div class="col-sm-1">
                                    <input type="reset" class="btn btn-secondary" value="Resetează">
                                </div>
                            </div>
                        </form>
